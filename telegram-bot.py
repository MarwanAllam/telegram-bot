...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ التحقق الحقيقي إن كان فيه دور شغال
...
async def collect_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "teacher":
        ...
        queues[chat_id] = {
            ...
            "creator_name": creator_name,  # حفظ الاسم
            "admins": set(),
            "members": [],
            "removed": set(),
            "all_joined": set(),
...
# 🛑 أمر /forceclose لقفل الدور إجباريًا (أي حد يستخدمه)
async def force_close(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    user_name = update.effective_user.full_name  # 👤 اسم اللي نفذ الأمر
